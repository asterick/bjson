...
function stringify(obj, cs) {
    ...
    if (cs.length < 2) {
        throw new Error("Character set must be at least 2 characters in length");
    }

    var bits = Math.floor(Math.log(cs.length) / Math.log(2));
    var arr = marshall(obj, bits);
